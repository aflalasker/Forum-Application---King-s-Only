...
class ViewForumFlowTest < ActionDispatch::IntegrationTest
    test 'user sees all topics and posts' do
        ...
        @user2 = User.create(first_name: "Jeroen", last_name: "Keppens", email: "jeroen.keppens@kcl.ac.uk", role: 1)
        ...
        topic = Topic.create(name: "Staff Topic", content: "Staff Content", category_id: category.id, user_id: @user2.id)
        post = Post.create(content: "Staff post content", topic_id: topic.id, user_id: @user2.id)
    ...
    test 'user cannot vote twice' do
        @user = User.create(first_name: "Alin", last_name: "Fulga", email: "alin.fulga@kcl.ac.uk", role: 0)
        @user2 = User.create(first_name: "Aflal", last_name: "Asker", email: "aflal.asker@kcl.ac.uk", role: 0)
        @user3 = User.create(first_name: "Jeroen", last_name: "Keppens", email: "jeroen.keppens@kcl.ac.uk", role: 1)
        page.set_rack_session(id: @user.id, email: @user.email, name: @user.last_name + " " + @user.first_name)
        category = Category.create(name: "General Discussions")
        topic = Topic.create(name: "General Topic", content: "General Content", category_id: category.id, user_id: @user.id)
        post = Post.create(content: "General post content", topic_id: topic.id, user_id: @user2.id)
        category = Category.create(name: "Staff Administration")
        topic = Topic.create(name: "Staff Topic", content: "Staff Content", category_id: category.id, user_id: @user3.id)
        post = Post.create(content: "Staff post content", topic_id: topic.id, user_id: @user3.id)
        visit '/categories/index'
        click_link('General Discussions')
        click_link('General Topic')
        click_link('+')
